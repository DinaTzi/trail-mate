...
2. `Emoji`
   ...
   - 需要一个小型内置 content supplement font：`builtin-emoji-core`。
...
4. 内建 emoji 内容字体补充：`builtin-emoji-core`
...
这个例外只用于小型、已编目的 content supplement。当前固件内置的 `builtin-emoji-core` 属于这个例外，因为它不读取 SD / Flash 外部 `font.bin`，只覆盖精选 emoji 候选集。这个例外不适用于：
...
### 4.5.2 Emoji Built-In Boundary

Emoji 支持不是一个 locale，也不是 Extensions 中的可下载包。
...
2. `builtin-emoji-core`
   - 小型内置 content supplement font。
...
1. 固件最多内置 100 个 emoji 候选。
2. 选择原则是高频、通用、跨语言语义稳定。
3. 如果未来要扩展到全量 emoji，必须另立新规格；不得悄悄扩大当前内置表。
