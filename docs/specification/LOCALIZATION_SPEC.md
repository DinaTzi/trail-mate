...
当前固件中的通用候选列表后端是 `builtin-candidate-picker`。它只定义“如何展示并提交一组 UTF-8 候选”，不拥有具体 emoji 表、语言词表或业务页面逻辑。候选型 IME 进入时必须打开共享全屏候选页；候选页最多展示 100 个候选，候选资源按预估使用频率排序，超出部分必须在资源层或 registry 层裁掉。
...
当前可启用输入法分成 locale 习惯输入法与基础/内容输入扩展：

1. 基础输入能力
   - `symbol-picker`
   - 后端：`builtin-candidate-picker`
   - 适用范围：常用特殊字符；不属于任何 locale。
2. `zh-Hans` / `zh-Hans-CN`
   - `zh-hans-pinyin`
   - 后端：`builtin-pinyin`
   - 适用范围：简体中文拼音输入。
3. `ru`
...
4. 已安装的内容输入扩展
   - `emoji-picker`
   - 后端：`builtin-candidate-picker`
   - 适用范围：emoji UTF-8 文本插入；不属于任何 locale，未安装扩展时不得显示。
...
3. 内建 IME：`symbol-picker`

English 与基础特殊字符输入必须在没有任何外部 pack 的情况下仍可工作。
...
2. `emoji-picker`
   ...
   - 作为通用候选列表 IME 后端暴露给共享 `ImeWidget`。
   - 与 `symbol-picker` 使用同一个全屏候选页；差异只在候选资源来源。
...
flowchart LR
  ...
  ChatCompose["Chat / contact compose"] --> SharedIme["Shared ImeWidget"]
  SettingsText["Settings text modal"] --> SharedIme
  SharedIme --> ImeRegistry["IME registry"]
  ImeRegistry --> EmojiIme["emoji-picker"]
  ImeRegistry --> SymbolIme["symbol-picker"]
  EmojiIme --> TextInsert["UTF-8 text insertion"]
  SymbolIme --> TextInsert
```
...
6. 在 Wi-Fi、Chat 或 Settings 页面各自实现符号/emoji picker。
...
6. 默认固件可以内建 `symbol-picker` 的小型 ASCII 特殊字符候选表；这是基础输入能力，不是 emoji 扩展，也不能扩大成固件内置 emoji 表。
