...
- `IME Pack`
  声明输入行为，例如简体中文拼音、直接提交目标脚本字符的键盘布局，或通用候选列表。
...
- 内建 IME pack：`symbol-picker`

因此，即使没有任何外部语言包存在，English 和基础特殊符号输入也始终可用。
...
- `symbol-picker`，后端为 `builtin-candidate-picker`，内建特殊字符候选表
- `zh-hans-pinyin`，后端为 `builtin-pinyin`
- `ru-cyrillic-keyboard`，后端为 `builtin-keyboard-layout`，布局为 `ru-cyrillic`
- `emoji-picker`，后端为 `builtin-candidate-picker`，候选表来自 emoji 扩展 payload
...
目前仓库 bundle 中存在两类 IME 声明：

1. locale bundle 可以声明专用于该 locale 的 IME。
2. content/input extension 可以声明不属于任何 locale 的 IME。

当前可下载 content/input extension 中，emoji 扩展声明：
...
- `symbol-picker` 是内建输入能力，不属于任何 locale；它存在是为了在未联网、未安装外部包时仍能输入 Wi-Fi 密码、PSK、频道名等常用特殊字符。
- `emoji-picker` 是 content/input 扩展，不属于任何 locale。安装后 compose widget 可以把它作为一个 script/input 模式轮换出来；未安装时不能显示这个 IME。
...
```text
packs/emoji/
  package.ini                   # package_type=content-bundle
  fonts/emoji-core/manifest.ini # usage=content
  ime/emoji-picker/manifest.ini # backend=builtin-candidate-picker
  ime/emoji-picker/candidates.txt
```

它解决的是“内容可显示”和“输入可插入”，不改变 `display_locale`。
`candidates.txt` 是输入资源的一部分，必须随扩展包安装；固件不得把这些候选项写死在代码里，也不得按 `emoji-picker` id 编写专属业务分支。候选文件最多发布 100 项，顺序即候选页展示顺序。
