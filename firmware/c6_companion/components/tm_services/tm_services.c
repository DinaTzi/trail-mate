                                   ...
                                   tm_c6_config_report_t* out_report)
{
    ...
    if (config->ble.ble_enabled)
    {
        ...
        s_ble_state = s_enabled_features & (TM_C6_FEATURE_BLE_MESHTASTIC |
                                            TM_C6_FEATURE_BLE_MESHCORE |
                                            TM_C6_FEATURE_BLE_TRAILMATE)
                          ? TM_C6_SERVICE_READY
                          : TM_C6_SERVICE_DISABLED;
    }
    else
    {
        s_ble_state = TM_C6_SERVICE_DISABLED;
    ...
    if (config->wifi.wifi_enabled)
    {
        ...
        s_wifi_state = (s_enabled_features & (TM_C6_FEATURE_WIFI_STA | TM_C6_FEATURE_WIFI_AP))
                           ? TM_C6_SERVICE_READY
                           : TM_C6_SERVICE_DISABLED;
    }
    else
...
{
    return send_frame(TM_C6_FRAME_LOG, TM_C6_CH_DIAG, (const uint8_t*)message, message ? strlen(message) : 0);
}
